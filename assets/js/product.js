...
window.addEventListener('resize', function() {
    if (window.innerWidth >= 992) {
        if (document.querySelector(".col-12-block").classList.contains("col-12")) {
            document.querySelector(".col-12-block").classList.remove("col-12");
            document.querySelector(".col-12-block").classList.add("col-9");
        }
    } else {
        if (document.querySelector(".col-12-block").classList.contains("col-9")) {
            document.querySelector(".col-12-block").classList.remove("col-9");
            document.querySelector(".col-12-block").classList.add("col-12");
        }
    }
});
...
const productRightImages = document.querySelectorAll(".product-right-img");
const productBigImg = document.querySelector(".product-big-img");

productRightImages.forEach(item => {
    item.addEventListener("mouseover",function(){
        productBigImg.src = item.src;
    })
});

const productMinusButton = document.querySelector(".product-minus-button");
const productplusButton = document.querySelector(".product-plus-button");
...
function updateTotalPrice() {
    ...
    productTotalPrice.innerText = `$${currentValue}00,000.000`;
}

productMinusButton.addEventListener("click", function () {
    let currentValue = parseInt(productQuantityInput.value);
    if (currentValue > 1) {
...
productplusButton.addEventListener("click", function () {
    let currentValue = parseInt(productQuantityInput.value);
    productQuantityInput.value = currentValue + 1;
    updateTotalPrice();
});
